...
if ($Request.Query.Extension -and $Request.Query.Type) {
    ...
    $table = Invoke-SqlQuery -Query "SELECT id, client_id, dialplan_name, dialplan_data, description FROM plugin_rader_ratel_custom_dialplan WHERE client_id=$cwaClientId GROUP BY dialplan_name
" -AsDataTable 
}
